 ...
 */
const checkStmt = db.prepare(`SELECT 1 FROM contacts WHERE account_id = ? AND user_id = ?`);

export function recordContactActivity(accountId: string, userId: string, displayName: string): boolean {
  if (!userId) return false; // payload thiếu uidFrom - không có gì để ghi
  const isNew = checkStmt.get(accountId, userId) === undefined;
